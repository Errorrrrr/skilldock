...
应用更新页每次进入时自动检查一次，不受后台 6 小时间隔限制；已有检查进行中时复用该请求。设置尚未保存时等待保存完成，安装期间不重复检查。
...
`skilldock install` 支持本地目录、Git 和网站来源，并通过重复 `--to` 一次入库后分发给多个工具。工具目录或同名内容存在歧义时不会自动覆盖；分发部分失败保留已完成步骤，返回成员 ID 与各目标结果供重试。随源码提供 Agent 接入 Skill 和规则模板，见 [Agent 统一安装入口](agent-cli-integration.md)。

`install` 输出从完整 Snapshot 改为安装报告，已有脚本获取完整状态时应使用 `list --json`；原 `exec` API 不变。发布继续使用既有桌面安装包流水线；CLI 按接入文档从源码安装，本版本不自动修改用户工具的全局规则。此前尚未提交的归集差异分类改动不在本次发布范围内。
